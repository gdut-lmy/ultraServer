 ...
 */
void test_fiber1() {
    ULTRA_LOG_INFO(g_logger) << "test_fiber1 begin";

    /**
     * 协程主动让出执行权，在yield之前，协程必须再次将自己添加到调度器任务队列中，
     * 否则yield之后没人管，协程会处理未执行完的逃逸状态，测试时可以将下面这行注释掉以观察效果
     */
    ultra::Scheduler::GetThis()->schedule(ultra::Fiber::GetThis());

    ULTRA_LOG_INFO(g_logger) << "before test_fiber1 yield";
    ultra::Fiber::GetThis()->yield();
    ULTRA_LOG_INFO(g_logger) << "after test_fiber1 yield";

    ULTRA_LOG_INFO(g_logger) << "test_fiber1 end";
}
 ...
 */
void test_fiber2() {
    ULTRA_LOG_INFO(g_logger) << "test_fiber2 begin";
    ...
    ULTRA_LOG_INFO(g_logger) << "test_fiber2 end";
}

void test_fiber3() {
    ULTRA_LOG_INFO(g_logger) << "test_fiber3 begin";
    ULTRA_LOG_INFO(g_logger) << "test_fiber3 end";
}

void test_fiber5() {
    static int count = 0;

    ULTRA_LOG_INFO(g_logger) << "test_fiber5 begin, i = " << count;
    ULTRA_LOG_INFO(g_logger) << "test_fiber5 end i = " << count;
 ...
 */
void test_fiber4() {
    ULTRA_LOG_INFO(g_logger) << "test_fiber4 begin";

    for (int i = 0; i < 3; i++) {
        ultra::Scheduler::GetThis()->schedule(test_fiber5, ultra::GetThreadId());
    }

    ULTRA_LOG_INFO(g_logger) << "test_fiber4 end";
}
...
int main() {
    ...
    ultra::Fiber::ptr fiber(new ultra::Fiber(&test_fiber3));
    sc.schedule(fiber);
    ...
    /**
     * 只要调度器未停止，就可以添加调度任务
     * 包括在子协程中也可以通过ultra::Scheduler::GetThis()->scheduler()的方式继续添加调度任务
     */

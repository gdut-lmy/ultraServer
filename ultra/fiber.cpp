...
namespace ultra {
    ...
    static Logger::ptr g_logger = ULTRA_LOG_NAME("system");
...
/// 线程局部变量，当前线程正在运行的协程,thread_local是为了每个线程有独立的变量，每个线程独有一份
 ...
 */
    ...
    Fiber::Fiber() {
        ...
        if (getcontext(&m_ctx)) {
            ULTRA_ASSERT2(false, "getcontext");
        }

        ++s_fiber_count;
        m_id = s_fiber_id++; // 协程id从0开始，用完加1

        ULTRA_LOG_DEBUG(g_logger) << "Fiber::Fiber() main id = " << m_id;
    }
 ...
 */
    Fiber::ptr Fiber::GetThis() {
        ...
        ULTRA_ASSERT(t_fiber == main_fiber.get());
 ...
 */
    Fiber::Fiber(std::function<void()> cb, size_t stacksize, bool run_in_scheduler)
        ...
        ULTRA_LOG_DEBUG(g_logger) << "Fiber::Fiber() id = " << m_id;
    }
 ...
 */
    Fiber::~Fiber() {
        ...
        if (m_stack) {
            ...
            ULTRA_ASSERT(m_state == TERM);
            StackAllocator::Dealloc(m_stack, m_stacksize);
            ULTRA_LOG_DEBUG(g_logger) << "dealloc stack, id = " << m_id;
        } else {
            // 没有栈，说明是线程的主协程
            ULTRA_ASSERT(!m_cb);              // 主协程没有cb
            ULTRA_ASSERT(m_state == RUNNING); // 主协程一定是执行状态
 ...
 */
    ...
    void Fiber::resume() {
        ULTRA_ASSERT(m_state != TERM && m_state != RUNNING);
        SetThis(this);
        m_state = RUNNING;
    ...
    void Fiber::yield() {
        ...
        ULTRA_ASSERT(m_state == RUNNING || m_state == TERM);
